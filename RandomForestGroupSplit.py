...
DesiredFeatures_no_var = ['PERCLOS','Gaze X Corrected','Gaze Y Corrected','Proband']
...
# Create the 'Proband' to group by proband 
class2 = class2.reset_index(drop = True)
class5 = class5.reset_index(drop = True)
class8 = class8.reset_index(drop = True)

class2proband = class2['Filename Input'].reset_index(drop = True)
class5proband = class5['Filename Input'].reset_index(drop = True)
class8proband = class8['Filename Input'].reset_index(drop = True)

for i in range(len(class2)):
    try:
        class2proband[i]=float(class2proband[i].split('_')[3]) + 100
    except ValueError:
        class2proband[i]= 50
for i in range(len(class5)):
    class5proband[i]=float(class5proband[i].split('_')[3]) + 100
for i in range(len(class8)):
    class8proband[i]=float(class8proband[i].split('_')[3]) + 100

class2['Proband'] = class2proband
class5['Proband'] = class5proband
class8['Proband'] = class8proband
...
probandgroup = RFmatrix['median Proband']
RFmatrix = RFmatrix.drop(columns=['median Proband'],axis=1).astype(float)
...
train_dataset,test_dataset = next(gss.split(RFmatrix_X, RFmatrix_y,probandgroup))
X_train = RFmatrix_X.iloc[list([train_dataset][0])]
X_test = RFmatrix_X.iloc[list([test_dataset][0])]
y_train = RFmatrix_y.iloc[list([train_dataset][0])]
y_test = RFmatrix_y.iloc[list([test_dataset][0])]
...
gkf = list(GroupKFold(n_splits=10).split(RFmatrix_X, RFmatrix_y, probandgroup))
...
if learningcurve:
    ...
    train_sizes, train_scores, validation_scores = learning_curve(estimator = RandomForestClassifier(n_estimators=1000, 
                                                                                                     ...
                                                                                                     n_jobs = -1, 
                                                                                                     random_state=rstate),
                                                                  X = X_shuf, y = Y_shuf, train_sizes = trainsizes, cv = gkf, n_jobs = -1)
    plt.figure()
...
if quick_run:
    ...
    clfrf = RandomForestClassifier(n_estimators=1000, min_samples_split=2,
    ...
    for i in range(len(gkf)):
        clfrf.fit(RFmatrix_X.iloc[gkf[i][0]],RFmatrix_y.iloc[gkf[i][0]])
        y_pred=clfrf.predict(RFmatrix_X.iloc[gkf[i][1]])
        y_test1=RFmatrix_y.iloc[gkf[i][1]]
        scoresCV.append(metrics.accuracy_score(y_test1, y_pred))

    scores = np.mean(scoresCV)
